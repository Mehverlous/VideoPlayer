...
static int save_audio_frame(AVFrame *frame){
    aud_frame_data[current_frame].audio_data[0] = frame->extended_data[0];
    aud_frame_data[current_frame].audio_samples = frame->nb_samples;
    aud_frame_data[current_frame].audio_format = frame->format;
    current_frame++;
...
static int output_audio_frame()
{
    ...
    for(int i = 0; i <= current_frame; i++){
        pa_err = Pa_WriteStream(pa_stream, aud_frame_data[current_frame].audio_data[0], aud_frame_data[current_frame].audio_samples * av_get_bytes_per_sample(aud_frame_data[current_frame].audio_format));
        if (pa_err != paNoError) {
